...
class VWAP(IndicatorBase):
    ...
    def __init__(
        self, name: str, symbol: str, timeframe: str, market_clock: MarketClock
    ) -> None:
    ...
    def update(self, candle: Candle) -> None:
        ...
        try:
            ...
            if self.current_session_date is None:
                self.current_session_date = candle_date

            elif candle_date != self.current_session_date:
                self.logger.info(f"{self.name} session reset triggered")
            ...
            if self.cumulative_volume == 0:
                self.logger.warning(f"{self.name} cumulative volume is zero")
            ...
            self.logger.error(f"VWAP update failed: {error}")
    ...
    def get_session_date(self) -> date | None:
        """
