...
from core.strategy.trade_decision import TradeDecision
...
class TradeDecisionEngine:
    ...
    def __init__(self) -> None:
        ...
        self.config = ScoringConfigLoader()

        self.minimum_score = float(
            self.config.get_position_sizing().get("minimum_trade_score", 50.0)
        )

    def evaluate(
        self, score: float, direction: str, confidence: str, quantity: int
    ) -> TradeDecision:
        """
        Determine whether
        ...
        try:
            ...
            if normalized_direction == "STRONG_LONG":
            ...
            self.logger.error(f"Trade decision failed: {error}")
